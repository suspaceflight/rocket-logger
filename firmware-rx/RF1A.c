...
// @return      none
// *****************************************************************************
void WriteSingleReg(uint8_t addr, uint8_t value) //done
{   
	CS_LOW;
	spi_tx(addr | RF_SNGLREGWR);	    // Send address + Instruction
	spi_tx(value); 			    // Write data in 
	CS_HIGH;
}
